...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are a travel itinerary parser. Given raw reservation/booking text, extract structured events.
...
- For flights with both departure and arrival info, create TWO events: one flight-departure and one flight-arrival.
- Omit fields that have no data (don't include null or empty strings).
- If you cannot determine a date, use today's date.
- Be thorough: extract every piece of useful information from the text.`;
